...
// The input is generated such that for updateBid and removeBid, the bid from the given userId for the given itemId will
// be valid.
...
class AuctionSystem {
 public:
  ...
  void addBid(int userId, int itemId, int bidAmount) {
    if (items.count(itemId) && items[itemId].u2b.count(userId)) {
      int old = items[itemId].u2b[userId];
      items[itemId].bu.erase({old, userId});
    }
    items[itemId].u2b[userId] = bidAmount;
    items[itemId].bu.insert({bidAmount, userId});
  }

  void updateBid(int userId, int itemId, int newAmount) {
    auto& item = items[itemId];

    int old = item.u2b[userId];
    item.bu.erase({old, userId});

    item.u2b[userId] = newAmount;
    item.bu.insert({newAmount, userId});
  }

  void removeBid(int userId, int itemId) {
    auto& item = items[itemId];

    int bid = item.u2b[userId];
    item.u2b.erase(userId);
    item.bu.erase({bid, userId});
  }

  int getHighestBidder(int itemId) {
    if (items.count(itemId) == 0) return -1;

    auto& item = items[itemId];

    if (item.bu.empty()) return -1;
    return item.bu.rbegin()->second;
  }

 private:
  struct Item {
    std::unordered_map<int, int> u2b;  // user:bid
    std::set<std::pair<int, int>> bu;  // bid:user
  };
  std::map<int, Item> items;  // item id : Item
};
